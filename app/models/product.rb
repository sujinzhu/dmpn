class Product < ApplicationRecord
  ...
  def self.create_many(category, product_names, batch, code_length, standard)
    Array.new(product_names.length){|i| self.create(category: category, code: get_id(category, code_length, standard), name: product_names[i], batch: batch)}
  ...
  def self.full_search_query(search='')
    ...
    private
    def self.get_id(category, code_length, standard)
      _code = category.next_id(code_length, standard)
      return _code unless exists?(code: _code)

      get_id(category, code_length, standard)
    end
  end
